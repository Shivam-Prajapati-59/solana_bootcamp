...
describe("Vesting Smart Contract Tests", () => {
  ...
  it("should claim tokens", async () => {
    ...
    // Derive the employee token account using the current mint
    [employeeTokenAccount] = PublicKey.findProgramAddressSync(
      [beneficiary.publicKey.toBuffer(), TOKEN_PROGRAM_ID.toBuffer(), mint.toBuffer()],
      ASSOCIATED_TOKEN_PROGRAM_ID,
    );

    // Create a new provider with beneficiary as signer for claim operation
    const beneficiaryProvider = new BankrunProvider(context);
    beneficiaryProvider.wallet = new anchor.Wallet(beneficiary);
    const beneficiaryProgram = new Program<Vesting>(IDL as Vesting, beneficiaryProvider);
